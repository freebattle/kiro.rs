...
/// 获取当天请求统计
pub async fn get_request_stats(State(state): State<AdminState>) -> impl IntoResponse {
    let stats = state.request_log.get_today_stats();
    Json(stats)
}
...
pub async fn get_usage_stats(
    ...
    let usage = state.usage_stats.get_month(&month);
    Json(usage)
}
